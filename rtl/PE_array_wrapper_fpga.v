...
assign result_we = output_we;
assign result_addr = output_addr;
assign result_data = output_data_in;
...
PE_array_8x16_tile UUT(
    .rst(rst), .clk(clk), .start(mac_start),
    .weight(weight_vec), .in(in_vec),
    .busy(mac_busy), .done(mac_done), .final_out(final_out));
            ...
            DMA_WAIT: begin
                if (dram_rvalid) begin
                    ...
                    if ((dma_mode == DMA_WEIGHT
                        ...
                        // The external interface is a single-word req/rvalid
                        // handshake, not a streaming burst.  Issue a fresh
                        // request for every word after consuming rvalid.
                        // Remaining in DMA_WAIT here deadlocks with a
                        // one-cycle rvalid pulse after the first word.
                        if (dma_burst_word_cnt == DMA_BURST_WORDS - 1)
                            dma_burst_word_cnt <= 0;
                        else
